...
import ImageModule from "./ExpoOpenCVModule";
import NativeImageViewWrapper from "./NativeImageView";
import {
  resolveContentFit,
...
export default class OpenCVImage extends React.PureComponent<OpenCVImageProps> {
  nativeViewRef: React.RefObject<NativeImageViewWrapper | null>;
  containerViewRef: React.RefObject<View | null>;

  constructor(props: OpenCVImageProps) {
    super(props);
   ...
   */
  static async generateThumbhashAsync(
    source: string | ImageRef,
  ): Promise<string> {
  ...
  render() {
    ...
    const isSFSymbol =
      ...
      resolvedSource.some((s) => s?.uri?.startsWith("sf:/"));
