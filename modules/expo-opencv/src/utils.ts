    ...
    switch (resizeMode) {
      ...
      case "repeat":
        if (!loggedRepeatDeprecationWarning) {
          console.log(
            '[expo-image]: Resize mode "repeat" is no longer supported',
          );
...
export function resolveContentPosition(
  ...
  if (typeof contentPosition === "string") {
    ...
    const contentPositionObject =
      contentPositionStringMappings[contentPosition];
...
export function resolveTransition(
  transition?: OpenCVImageProps["transition"],
  fadeDuration?: OpenCVImageProps["fadeDuration"],
